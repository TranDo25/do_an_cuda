...
int main() {
	...
	std::cout << "====================================================" << std::endl;
    auto start_time = std::chrono::high_resolution_clock::now(); // Lấy thời điểm bắt đầu

	Point* result = OuterConvexApproximation(points, n_poly);
    auto end_time = std::chrono::high_resolution_clock::now(); // Lấy thời điểm kết thúc

    // Tính thời gian thực thi (đơn vị: microseconds)
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();

    // In ra thời gian thực thi
    std::cout << "Thoi gian thuc thi cua OuterConvexApproximation voi n = "<< n_poly<< "la: " << duration << " microseconds" << std::endl;
